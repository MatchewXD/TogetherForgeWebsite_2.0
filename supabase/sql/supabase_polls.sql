...
begin
  ...
  if new.status = 'live' and (tg_op = 'INSERT' or old.status is distinct from 'live') then
    select count(*) into v_n from public.poll_options where poll_id = new.id;
    if v_n < 2 or v_n > 8 then
      raise exception 'A live poll needs between 2 and 8 options.';
    end if;
...
begin
  ...
  select count(*) into v_n from public.poll_options where poll_id = v_poll;

  if v_n > 8 then
    raise exception 'A poll can have at most 8 options.';
  end if;

  if coalesce(v_status, 'draft') is distinct from 'draft' and v_n < 2 then
    raise exception 'A poll needs at least 2 options.';
  end if;

  if coalesce(v_status, 'draft') = 'live' and tg_op in ('INSERT', 'DELETE', 'UPDATE') then
    raise exception 'Options are locked while a poll is live. Close it to stop voting.';
  end if;

  if coalesce(v_status, 'draft') = 'closed' and tg_op in ('INSERT', 'DELETE', 'UPDATE') then
    raise exception 'Closed poll options cannot change.';
  end if;
...
create trigger trg_enforce_poll_option_count
  after insert or delete or update of poll_id, name, description
