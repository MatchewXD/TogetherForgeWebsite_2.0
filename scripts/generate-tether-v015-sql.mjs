/**
 * Writes supabase/sql/supabase_tether_task_tree_v015.sql from the v0.15 tree.
 * Staging-only upsert for Tether-7 (plus Tether-11 UI if present).
 * Run: node scripts/generate-tether-v015-sql.mjs
 */
import { writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  TETHER_V015_PROJECT_SLUG,
  TETHER_V015_TASKS,
  TETHER_V015_VERSION,
  buildTetherV015Description,
  tetherV015Difficulty,
  tetherV015StaffOnly,
  tetherV015Subtasks,
  tetherV015Title,
} from '../src/data/tetherTaskTreeV015.js';
...
const rows = TETHER_V015_TASKS.map((task) => {
  const title = tetherV015Title(task);
  if (title.length > 120) {
    throw new Error(`Title over 120 chars: ${title}`);
  }
  const desc = buildTetherV015Description(task);
  if (desc.length > 2000) {
    throw new Error(`Description over 2000 chars: ${task.code} (${desc.length})`);
  }
  if (task.state === 'Parked') {
    throw new Error(`Parked is not allowed in v0.15: ${task.code}`);
  }
  if (task.code === 'Tether-5.6' || String(task.code).startsWith('Tether-5.6.')) {
    throw new Error(`Tether-5.6 is out of this pass: ${task.code}`);
  }
  if (String(task.code).startsWith('Tether-4') || String(task.code).startsWith('Tether-5') || String(task.code).startsWith('Tether-6')) {
    throw new Error(`Tether-4/5/6 are out of this pass: ${task.code}`);
  }
  const depth = (() => {
  ...
  return {
    ...
    description: desc,
    category: task.skill,
    difficulty: tetherV015Difficulty(task.size),
    estimatedEffort: task.size,
    staffOnly: tetherV015StaffOnly(task),
    sortOrder: task.sortOrder,
    blockedByCodes: task.blockedByCodes || (task.blockedByCode ? [task.blockedByCode] : []),
    subtasks: tetherV015Subtasks(task),
    status: 'ToDo',
  };
});
...
const sql = `-- Tether-7 Stats, weapons, tools from Tether_Task_Breakdown_${TETHER_V015_VERSION}
...
begin
  ...
  where slug = ${sqlStr(TETHER_V015_PROJECT_SLUG)}
  limit 1;

  if v_project is null then
    raise exception 'Tether project not found (slug=${TETHER_V015_PROJECT_SLUG}).';
  end if;
...
const out = join(root, 'supabase/sql/supabase_tether_task_tree_v015.sql');
