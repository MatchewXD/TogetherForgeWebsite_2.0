/**
 * Writes supabase/sql/supabase_tether_task_tree_v021.sql from the v0.21 tree.
 * Staging-only upsert for Tether-8 UI and Tether-11 animation.
 * Run: node scripts/generate-tether-v021-sql.mjs
 */
import { writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  TETHER_V021_PROJECT_SLUG,
  TETHER_V021_TASKS,
  TETHER_V021_VERSION,
  buildTetherV021Description,
  tetherV021Difficulty,
  tetherV021StaffOnly,
  tetherV021Subtasks,
  tetherV021Title,
} from '../src/data/tetherTaskTreeV021.js';
...
const rows = TETHER_V021_TASKS.map((task) => {
  const title = tetherV021Title(task);
  if (title.length > 120) {
    throw new Error(`Title over 120 chars: ${title}`);
  }
  const desc = buildTetherV021Description(task);
  if (desc.length > 2000) {
    throw new Error(`Description over 2000 chars: ${task.code} (${desc.length})`);
  }
  if (task.state === 'Parked') {
    throw new Error(`Parked is not allowed in v0.21: ${task.code}`);
  }
  if (/^Tether-[4-7]([.]|$)/.test(task.code)) {
    throw new Error(`Tether-4/5/6/7 are out of this pass: ${task.code}`);
  }
  ...
  return {
    ...
    description: desc,
    category: task.skill,
    difficulty: tetherV021Difficulty(task.size),
    estimatedEffort: task.size,
    staffOnly: tetherV021StaffOnly(task),
    sortOrder: task.sortOrder,
    blockedByCodes: blockers,
    subtasks: tetherV021Subtasks(task),
    status: 'ToDo',
  };
});
...
const sql = `-- Tether-8 UI and Tether-11 animation from Tether_Task_Breakdown_${TETHER_V021_VERSION}
...
begin
  ...
  where slug = ${sqlStr(TETHER_V021_PROJECT_SLUG)}
  limit 1;

  if v_project is null then
    raise exception 'Tether project not found (slug=${TETHER_V021_PROJECT_SLUG}).';
  end if;
...
const out = join(root, 'supabase/sql/supabase_tether_task_tree_v021.sql');
